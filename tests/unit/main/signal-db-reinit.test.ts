...
describe('Signal Database Re-initialization', () => {
  ...
  describe('save-device-identity with private key', () => {
    ...
    it('should handle database initialization failure by throwing error', async () => {
      ...
      // Act & Assert
      await expect(saveDeviceIdentityHandler(mockEvent, deviceIdentity)).rejects.toThrow('Database initialization failed');
    ...
    it('should handle pre-key upload failure by throwing error', async () => {
      ...
      // Act & Assert
      await expect(saveDeviceIdentityHandler(mockEvent, deviceIdentity)).rejects.toThrow('Network error');
  ...
  if (private_key !== undefined) {
    ...
    if (authData && authData.user_id && authData.device_id) {
      ...
      try {
        ...
        const mockSignalDb = (openSignalDatabase as jest.Mock)('/tmp/test-userdata', privateKeyBytes, {});
