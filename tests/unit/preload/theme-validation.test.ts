...
describe('Theme Settings Validation', () => {
  ...
  function applyThemeSync(savedTheme: any): void {
    if (
      ...
      console.log("warn", "Theme settings failed validation in preload; skipping early application");
    }
  }
    ...
    it('should reproduce theme settings validation failure with invalid accent RGB', () => {
      ...
      expect(mockLog).toEqual(
        expect.arrayContaining([
          expect.arrayContaining(["warn", "Theme settings failed validation in preload; skipping early application"])
        ])
      );
    });
  ...
  describe('Validation Success', () => {
    it('should successfully apply valid theme settings', () => {
      ...
      console.log('All setProperty calls:', mockDocument.documentElement.style.setProperty.mock.calls);
      
      // Check that all expected properties were set
      expect(mockDocument.documentElement.style.setProperty).toHaveBeenNthCalledWith(1, "--rgb-highlight", "255,0,0");
      expect(mockDocument.documentElement.style.setProperty).toHaveBeenNthCalledWith(2, "--rgb-background", "0,0,0");
      expect(mockDocument.documentElement.style.setProperty).toHaveBeenNthCalledWith(3, "--rgb-surface", "255,255,255");
      expect(mockDocument.documentElement.style.setProperty).toHaveBeenNthCalledWith(4, "--rgb-surface-hover", "255, 255, 255");
      expect(mockDocument.documentElement.style.setProperty).toHaveBeenNthCalledWith(5, "--chat-color", "#ffffff");
    });
  });
