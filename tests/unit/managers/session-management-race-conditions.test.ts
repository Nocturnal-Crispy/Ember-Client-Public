...
class MockSignalSessionManager {
  ...
  async encrypt(recipientAddress: string, plaintext: Uint8Array): Promise<{ ciphertext: Uint8Array; messageType: number }> {
    await new Promise(resolve => setTimeout(resolve, this.operationDelay));
  ...
  async decrypt(senderAddress: string, ciphertext: Uint8Array, messageType: number): Promise<Uint8Array> {
    await new Promise(resolve => setTimeout(resolve, this.operationDelay));
...
class TestDirectMessagingManager {
  ...
  async startDmConversation(
    ...
    try {
      ...
      this.signalManager.ensureSession(participantId, partnerDeviceId).catch(
        (err: Error) => console.warn("Signal ensureSession failed", err),
      );
