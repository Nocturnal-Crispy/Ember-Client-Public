...
(function (): void {
  ...
  async function joinVoiceChannel(
    ...
    const VoiceManagerClass = (window as any).VoiceManager as new (
      ws: WebSocket,
      auth: AuthForVoice
    ...
    const joined = await (
      App.voiceManager as {
        joinChannel(id: string, s: VoiceSettings | null): Promise<boolean>;
      }
  ...
  function renderVoiceParticipants(channelId: string | null): void {
    ...
    const list = document.querySelector<HTMLElement>(
      `.voice-participant-list[data-voice-channel-id="${channelId}"]`
    );
    if (!list) return;
  ...
  async function toggleCamera(): Promise<void> {
    ...
    try {
      if (!App.localCameraOn) {
        ...
        const joined = await (
          App.voiceManager as {
            enableCamera(id: string | null): Promise<boolean>;
          }
