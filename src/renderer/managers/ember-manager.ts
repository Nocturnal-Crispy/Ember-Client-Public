...
(function (): void {
  ...
  function setCryptoState(
    emberId: string,
    update: Partial<ConversationCryptoState>
    ...
    const next: ConversationCryptoState = {
      ...
      activeDistributionId:
        update.activeDistributionId !== undefined
          ? update.activeDistributionId
          : current.activeDistributionId,
  ...
  const senderKeyOperationLog = new Map<
    string,
    {
      createdAt: number;
  ...
  async function createSenderKeyForEmber(
    ...
    try {
      ...
      const response = await window.emberAPI.invoke<{
        distributionMessage: string;
        error?: string;
      }>('CreateSenderKeyDistribution', { distributionId });
      ...
      if (err.message.includes('Signal database not available')) {
        throw new Error(
          'Signal database not available - please check database configuration and restart the application'
        );
      }
  ...
  async function ensureSignalSession(
    ...
    const sessionResponse = await window.emberAPI.invoke<{ record: string | null }>('LoadSession', {
      address,
    });
      ...
      try {
        ...
        if (bundleResponse.ok) {
          ...
          if (!pkbResult.success) {
            throw new Error(
              `ProcessPreKeyBundle failed for ${address}: ${pkbResult.error ?? 'unknown'}`
            );
          }
          ...
          throw new Error(
            `Failed to fetch self pre-key bundle for ${address} (HTTP ${bundleResponse.status})`
          );
        }
      } catch (error) {
        const err = error as Error;
    ...
    while (retries < maxRetries) {
      try {
        ...
        if (bundleResponse.ok) {
          ...
          throw new Error(
            `Failed to fetch pre-key bundle for ${address} (HTTP ${bundleResponse.status})`
          );
        }
      } catch (error) {
        retries++;
  ...
  async function distributeSenderKeyToMembers(emberId: string, auth: AuthData): Promise<void> {
    ...
    try {
      ...
      if (distributions.length > 0) {
        await fetch(`${auth.hostname}/api/v1/embers/${emberId}/sender-key-distributions`, {
          method: 'POST',
          headers: {
  ...
  async function processIncomingSenderKeyDistributions(): Promise<void> {
    try {
      ...
      if (!auth) return;
      const response = await fetch(`${auth.hostname}/api/v1/sender-key-distributions/pending`, {
        headers: { Authorization: `Bearer ${auth.token}` },
      });
      ...
      for (const dist of pending) {
        try {
          ...
          const decResponse = await window.emberAPI.invoke<{ plaintext: string }>(decryptCmd, {
            senderAddress,
          ...
          await fetch(`${auth.hostname}/api/v1/sender-key-distributions/${dist.id}/ack`, {
            method: 'POST',
            headers: { Authorization: `Bearer ${auth.token}` },
          });
          processed++;
    ...
    embers.forEach((ember, index) => {
      ...
      if (index === 0 && !App.activeEmberId) {
        ...
        loadServerContent(ember.id, ember.name).catch((err: Error) =>
          log.error('Failed to load server content on render', {
            ember_id: ember.id,
            error: err.message,
          })
        );
  ...
  function switchToServer(emberId: string, emberName: string): void {
    ...
    loadServerContent(emberId, emberName).catch((err: Error) =>
      log.error('Failed to load server content on switch', {
        ember_id: emberId,
        error: err.message,
      })
    );
  ...
  async function loadServerContent(emberId: string, emberName: string): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        alert(
          'Security Alert: Cryptographic state corruption detected. Please restart the application.'
        );
      ...
      log.warn(
        'Sender key setup deferred — crypto unavailable, loading content without encryption support',
        {
          ember_id: emberId,
          error: errorMessage,
        }
    ...
    try {
      ...
      log.warn('Failed to sync crypto state from server', {
        ember_id: emberId,
        error: (cryptoErr as Error).message,
      });
    }
  }
  ...
  const editEmberIconUpload = document.getElementById(
    'edit-ember-icon-upload'
  ) as HTMLInputElement | null;
  const editUploadIconBtn = document.getElementById('edit-upload-icon-btn');
  const editEmberIconUrl = document.getElementById(
    'edit-ember-icon-url'
  ) as HTMLInputElement | null;
  ...
  const editEmberSaveBtn = document.getElementById(
    'edit-ember-save-btn'
  ) as HTMLButtonElement | null;
  const editEmberCancelBtn = document.getElementById(
    'edit-ember-cancel-btn'
  ) as HTMLButtonElement | null;
  ...
  function updateEditIconSourceUI(): void {
    editIconToggleBtns.forEach(btn => {
      btn.classList.toggle('active', btn.dataset['source'] === editCurrentIconSource);
    });
  ...
  function updateEditIconPreview(data: string | null): void {
    ...
    while (editIconPreview.firstChild) editIconPreview.removeChild(editIconPreview.firstChild);
    if (data) {
      const img = document.createElement('img');
      img.src = data;
      img.onerror = () => {
  ...
  editIconToggleBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      editCurrentIconSource = (btn.dataset['source'] ?? 'upload') as 'upload' | 'url';
      updateEditIconSourceUI();
      App.currentIconData = null;
      updateEditIconPreview(null);
    });
  ...
  async function ensureSenderKeyForEmber(emberId: string): Promise<string | null> {
    try {
      ...
      log.warn('ensureSenderKeyForEmber failed', {
        ember_id: emberId,
        error: (err as Error).message,
      });
      return null;
