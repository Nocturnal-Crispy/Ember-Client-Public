...
async function performHealthcheck(): Promise<void> {
  try {
    const auth = await ipcRenderer.invoke('get-auth') as { hostname?: string; token?: string } | null;
...
async function updateUserStatus(apiStatus: string, displayStatus: string): Promise<void> {
  log.debug('Updating user status', { status: apiStatus });
  try {
    const auth = await ipcRenderer.invoke('get-auth') as { token?: string; hostname?: string; user_id?: string; username?: string } | null;
...
function updateUserPanelStatusColor(status: string): void {
  ...
  const userStatusIcon = document.getElementById('user-status-icon') as HTMLImageElement | null;
  if (userStatusIcon) userStatusIcon.src = iconSrc;
  const menuStatusIcon = document.getElementById('menu-status-icon') as HTMLImageElement | null;
