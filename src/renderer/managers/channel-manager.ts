...
(function (): void {
    ...
    function appendChannel(channel: Channel): void {
      ...
      channelEl.addEventListener("dragover", (e: DragEvent) => {
        ...
        channelEl.classList.add(
          insertAfter ? "drag-over-bottom" : "drag-over-top"
        );
      });
      ...
      channelEl.addEventListener("drop", async (e: DragEvent) => {
        ...
        if (
          !App.dragItem ||
          App.dragItem.type !== "channel" ||
          App.dragItem.id === channel.id
        )
          return;
        const rect = channelEl.getBoundingClientRect();
        const insertAfter = e.clientY > rect.top + rect.height / 2;
  ...
  async function reorderChannels(
    ...
    const allChannelEls = Array.from(
      document.querySelectorAll<HTMLElement>(".channel")
    );
    ...
    try {
      await fetch(
        `${auth.hostname}/api/v1/embers/${App.activeEmberId}/channels/reorder`,
        {
          method: "PUT",
          headers: {
      ...
      const { channels, categories: cats } = await fetchChannelsAndCategories(
        App.activeEmberId
      );
  ...
  async function reorderCategories(
    ...
    const allCatEls = Array.from(
      document.querySelectorAll<HTMLElement>(".channel-category")
    );
    ...
    try {
      await fetch(
        `${auth.hostname}/api/v1/embers/${App.activeEmberId}/categories/reorder`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  function showChannelContextMenu(
    ...
    if (editLabel)
      editLabel.textContent =
        target.type === "category" ? "Edit Category" : "Edit Channel";
    const deleteLabel = document.getElementById("ctx-delete-label");
    if (deleteLabel)
      deleteLabel.textContent =
        target.type === "category" ? "Delete Category" : "Delete Channel";
  ...
  document
    .getElementById("ctx-new-text-channel")
  ...
  document
    .getElementById("ctx-new-voice-channel")
    ?.addEventListener("click", (e: Event) => {
      e.stopPropagation();
      const categoryId = App.contextMenuTarget?.categoryId ?? null;
      hideChannelContextMenu();
  ...
  document
    .getElementById("ctx-edit-item")
    ?.addEventListener("click", (e: Event) => {
      e.stopPropagation();
      if (!App.contextMenuTarget) return;
      const mode =
        App.contextMenuTarget.type === "category"
          ? "edit-category"
          : "edit-channel";
  ...
  document
    .getElementById("ctx-new-category")
    ?.addEventListener("click", (e: Event) => {
      e.stopPropagation();
      hideChannelContextMenu();
  ...
  document
    .getElementById("ctx-delete-item")
    ?.addEventListener("click", (e: Event) => {
      e.stopPropagation();
    ...
    ?.addEventListener("click", async () => {
      ...
      try {
        ...
        if (ctxTarget.type === "category") {
          ...
          res = await fetch(
            `${auth.hostname}/api/v1/channels/${ctxTarget.id}`,
            {
              method: "DELETE",
              headers: { Authorization: `Bearer ${auth.token}` },
            }
        ...
        if (res.ok) {
          ...
          if (
            ctxTarget.type === "channel" &&
            ctxTarget.id === App.activeChannelId
          ) {
          ...
          document
            .getElementById("delete-confirm-modal")
            ?.classList.add("hidden");
          const { channels, categories: cats } =
            await fetchChannelsAndCategories(App.activeEmberId!);
          renderChannels(channels, cats);
        } else {
          const err = (await res.json().catch(() => ({}))) as {
            error?: string;
  ...
  const channelModalCancelBtn = document.getElementById(
    "channel-modal-cancel-btn"
  );
  const channelModalError = document.getElementById("channel-modal-error");

  const CHANNEL_MODAL_CONFIG: Record<
    string,
  ...
  channelModalConfirmBtn?.addEventListener("click", async () => {
    ...
    try {
      ...
      if (
        App.channelModalMode === "create-text" ||
        App.channelModalMode === "create-voice"
      ) {
        ...
        const res = await fetch(
          `${auth.hostname}/api/v1/embers/${App.activeEmberId}/channels`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        const res = await fetch(
          `${auth.hostname}/api/v1/channels/${App.channelModalTargetId}`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
        ...
        const res = await fetch(
          `${auth.hostname}/api/v1/embers/${App.activeEmberId}/categories`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
