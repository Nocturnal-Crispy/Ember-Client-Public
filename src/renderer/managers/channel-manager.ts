...
(function (): void {
  ...
  function renderChannels(channels: Channel[], categories: Category[] = []): void {
    ...
    interface AutoSelectEntry { el: HTMLElement; channel: Channel }
  ...
  function showChannelContextMenu(x: number, y: number, target: ContextMenuTarget): void {
    ...
    if (editLabel) editLabel.textContent = target.type === 'category' ? 'Edit Category' : 'Edit Channel';
    const deleteLabel = document.getElementById('ctx-delete-label');
    if (deleteLabel) deleteLabel.textContent = target.type === 'category' ? 'Delete Category' : 'Delete Channel';
  ...
  const channelModalConfirmBtn = document.getElementById('channel-modal-confirm-btn') as HTMLButtonElement | null;
  ...
  function openChannelNameModal(mode: string, categoryId: string | null, targetId: string | null, currentName: string, currentDescription = ''): void {
  ...
  channelModalConfirmBtn?.addEventListener('click', async () => {
    ...
    if (!name) { showChannelModalError('Name is required'); return; }
    if (channelModalConfirmBtn) channelModalConfirmBtn.disabled = true;
    try {
      const auth = await ipcRenderer.invoke('get-auth') as { token?: string; hostname?: string } | null;
      if (!auth || !auth.token || !auth.hostname) { showChannelModalError('Not authenticated'); return; }
      ...
      if (App.channelModalMode === 'create-text' || App.channelModalMode === 'create-voice') {
        ...
        if (!res.ok) { const err = await res.json().catch(() => ({})) as { error?: string }; throw new Error(err.error ?? 'Failed to create channel'); }
        ...
        if (!res.ok) { const err = await res.json().catch(() => ({})) as { error?: string }; throw new Error(err.error ?? 'Failed to update channel'); }
        ...
        if (!res.ok) { const err = await res.json().catch(() => ({})) as { error?: string }; throw new Error(err.error ?? 'Failed to create category'); }
        ...
        if (!res.ok) { const err = await res.json().catch(() => ({})) as { error?: string }; throw new Error(err.error ?? 'Failed to update category'); }
      }
