...
(function (): void {
  ...
  const joinInviteInput    = document.getElementById('join-invite-input') as HTMLInputElement | null;
  const joinServerBtn      = document.getElementById('join-server-btn') as HTMLButtonElement | null;
  ...
  function openJoinServerModal(): void {
    ...
    if (joinServerBtn) { joinServerBtn.disabled = false; joinServerBtn.textContent = 'Join'; }
  ...
  function showJoinServerError(message: string): void {
    if (joinServerError) { joinServerError.textContent = message; joinServerError.classList.remove('hidden'); }
  }

  joinServerCancelBtn?.addEventListener('click', closeJoinServerModal);
  joinServerModal?.addEventListener('click', (e: Event) => { if (e.target === joinServerModal) closeJoinServerModal(); });

  interface ParsedInvite { code: string; hostname: string | null }
  ...
  joinServerBtn?.addEventListener('click', async () => {
    ...
    if (!value.trim()) { showJoinServerError('Please enter an invite link or code'); return; }
    const parsed = parseInviteInput(value);
    if (!parsed) { showJoinServerError('Invalid invite link or code'); return; }
    if (joinServerBtn) { joinServerBtn.disabled = true; joinServerBtn.textContent = 'Loading...'; }
  ...
  if (serverHeader && serverHeaderMenu) {
    ...
    document.addEventListener('click', (e: Event) => {
      if (!serverHeaderMenu.classList.contains('hidden') && !serverHeaderMenu.contains(e.target as Node)) {
  ...
  const createInviteBtn       = document.getElementById('create-invite-btn') as HTMLButtonElement | null;
  const createInviteCancelBtn = document.getElementById('create-invite-cancel-btn');
  const inviteExpirationSelect = document.getElementById('invite-expiration') as HTMLSelectElement | null;
  const inviteMaxUsesSelect   = document.getElementById('invite-max-uses') as HTMLSelectElement | null;
  const inviteLinkResult      = document.getElementById('invite-link-result');
  const inviteLinkInput       = document.getElementById('invite-link-input') as HTMLInputElement | null;
  const inviteCopyBtn         = document.getElementById('invite-copy-btn') as HTMLButtonElement | null;
  ...
  function resetCreateInviteForm(): void {
    ...
    if (createInviteBtn) { createInviteBtn.disabled = false; createInviteBtn.textContent = 'Generate Link'; }
    hideCreateInviteError();
  }

  function showCreateInviteError(message: string): void {
    if (createInviteError) { createInviteError.textContent = message; createInviteError.classList.remove('hidden'); }
  }
  ...
  createInviteModal?.addEventListener('click', (e: Event) => { if (e.target === createInviteModal) closeCreateInviteModal(); });
    ...
    if (inviteLinkInput?.value) {
      navigator.clipboard.writeText(inviteLinkInput.value).then(() => {
        if (inviteCopyBtn) {
          ...
          setTimeout(() => { if (inviteCopyBtn) inviteCopyBtn.textContent = 'Copy'; }, 2000);
        }
      });
    }
  });
  ...
  async function handleCreateInvite(): Promise<void> {
    if (!App.activeEmberId) { log.warn('Cannot create invite: no active ember'); showCreateInviteError('No server selected'); return; }
    const emberKey = App.emberKeyCache.get(App.activeEmberId);
    if (!emberKey) { log.error('Cannot create invite: ember key not in cache', { ember_id: App.activeEmberId }); showCreateInviteError('Ember key not available'); return; }

    log.info('Creating invite', { ember_id: App.activeEmberId });
    try {
      if (createInviteBtn) { createInviteBtn.disabled = true; createInviteBtn.textContent = 'Generating...'; }
      const auth = await ipcRenderer.invoke('get-auth') as { token?: string; hostname?: string } | null;
      if (!auth || !auth.token || !auth.hostname) { showCreateInviteError('Not authenticated'); return; }
  ...
  const acceptInviteJoinBtn   = document.getElementById('accept-invite-join-btn') as HTMLButtonElement | null;
  ...
  function openAcceptInviteModal(inviteInfo: Record<string, unknown>): void {
    ...
    if (acceptInviteJoinBtn) { acceptInviteJoinBtn.disabled = false; acceptInviteJoinBtn.textContent = 'Join Server'; }
  ...
  function showAcceptInviteError(message: string): void {
    if (acceptInviteError) { acceptInviteError.textContent = message; acceptInviteError.classList.remove('hidden'); }
  }

  acceptInviteCancelBtn?.addEventListener('click', closeAcceptInviteModal);
  acceptInviteModal?.addEventListener('click', (e: Event) => { if (e.target === acceptInviteModal) closeAcceptInviteModal(); });
  ...
  async function handleAcceptInvite(): Promise<void> {
    ...
    try {
      if (acceptInviteJoinBtn) { acceptInviteJoinBtn.disabled = true; acceptInviteJoinBtn.textContent = 'Joining...'; }
      const auth = await ipcRenderer.invoke('get-auth') as { token?: string; hostname?: string } | null;
      const device = await ipcRenderer.invoke('get-device-identity') as { public_key?: string; private_key?: string } | null;
      if (!auth || !auth.token || !device) { showAcceptInviteError('Not authenticated'); return; }
      ...
      if (!emberKey) { log.error('Failed to decrypt ember key from invite'); showAcceptInviteError('Failed to decrypt ember key from invite'); return; }
      ...
      const data = await response.json() as { ember_id?: string; ember_name?: string };
      if (data.ember_id) {
  ...
  async function processInviteLink(code: string, hostname: string | null): Promise<void> {
    ...
    try {
      const auth = await ipcRenderer.invoke('get-auth') as { token?: string; hostname?: string } | null;
      if (!auth || !auth.token) { log.error('Cannot process invite link: not authenticated'); return; }
  ...
  ipcRenderer.on('handle-invite-link', (_event: unknown, ...args: unknown[]) => {
