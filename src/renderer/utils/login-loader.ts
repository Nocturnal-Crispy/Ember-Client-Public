...
(function (): void {
  ...
  async function fetchFragment(name: string): Promise<string> {
    ...
    if (!response.ok) {
      throw new Error(
        `[login-loader] Failed to fetch fragment "${name}": HTTP ${response.status}`
      );
    }
  ...
  function loadScript(src: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      el.onload = () => resolve();
      el.onerror = () =>
        reject(new Error(`[login-loader] Failed to load script: ${src}`));
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    try {
      const [titleBar, loginForm, modalRecoveryCode, overlayLoading] =
        await Promise.all([
