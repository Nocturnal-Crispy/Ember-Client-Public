      ...
      App.wsConnection.onmessage = (event: MessageEvent) => {
        try {
          ...
          if (data.type === 'new_message' && data.payload) {
            ...
            if (App.voiceManager) (App.voiceManager as { handleMessage(d: unknown): void }).handleMessage(data);
            ...
            window.handleVoiceUserJoined(data.payload as Parameters<typeof window.handleVoiceUserJoined>[0]);
            ...
            window.handleVoiceUserLeft(data.payload as Parameters<typeof window.handleVoiceUserLeft>[0]);
          }
        } catch (err) {
