...
(function () {
    ...
    async function joinVoiceChannel(channelId, channelName) {
        ...
        if (App.activeVoiceChannelId && App.activeVoiceChannelId !== channelId && App.voiceManager) {
            ...
            await App.voiceManager.leaveChannel();
        ...
        const VoiceManagerClass = window.VoiceManager;
        if (!App.voiceManager) {
            App.voiceManager = new VoiceManagerClass(App.wsConnection, auth);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const vm = App.voiceManager;
            vm.onSpeakingChanged = (userId, isSpeaking) => updateSpeakingIndicator(userId, isSpeaking);
            vm.onParticipantsChanged = (participants) => {
                App.voiceParticipants.clear();
                participants.forEach((p) => App.voiceParticipants.set(p.user_id, p.username));
                renderVoiceParticipants(App.activeVoiceChannelId);
            };
            vm.onCameraStateChanged = (userId, isOn) => handleRemoteCameraStateChanged(userId, isOn);
            vm.onVideoStreamAdded = (streamId, stream) => handleRemoteVideoStream(streamId, stream);
        }
        else {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const vm = App.voiceManager;
        ...
        const joined = await App.voiceManager.joinChannel(channelId, voiceSettings);
        if (!joined) {
    ...
    function handleVoiceUserLeft(payload) {
        ...
        const selfId = App.voiceManager?.auth?.user_id;
        if (user_id !== selfId)
            playVoiceSound('userLeave');
    }
    ...
    document.getElementById('voice-mute-btn')?.addEventListener('click', () => {
        if (!App.voiceManager)
            return;
        const muted = App.voiceManager.toggleMute();
    ...
    document.getElementById('voice-deafen-btn')?.addEventListener('click', () => {
        if (!App.voiceManager)
            return;
        const deafened = App.voiceManager.toggleDeafen();
    ...
    async function toggleCamera() {
        if (!App.voiceManager || !App.activeVoiceChannelId)
            return;
        ...
        try {
            if (!App.localCameraOn) {
                ...
                const joined = await App.voiceManager.enableCamera(cameraDeviceId);
                if (!joined)
                    return;
                ...
                await App.voiceManager.disableCamera();
                App.localCameraOn = false;
    ...
    function renderVideoGrid() {
        ...
        App.voiceParticipants.forEach((username, userId) => {
            ...
            if (hasCamera && vm?.remoteVideoStreams) {
                vm.remoteVideoStreams.forEach(s => { if (!stream)
                    stream = s; });
            }
    ...
    function playVoiceSound(type) {
        if (App._vvSounds && App._vvSounds[type] === false)
            return;
        ...
        try {
            ...
            devices.forEach(d => {
                ...
                if (d.kind === 'audioinput')
                    _addOption(inputSel, d.deviceId, label);
                if (d.kind === 'audiooutput')
                    _addOption(outputSel, d.deviceId, label);
            });
        }
        catch (e) {
    ...
    async function populateVoiceVideoSettings() {
        try {
            ...
            const setVal = (id, val) => { const el = document.getElementById(id); if (el)
                el.value = String(val ?? ''); };
            const setChecked = (id, val) => { const el = document.getElementById(id); if (el)
                el.checked = val; };
            ...
            if (ivv)
                ivv.textContent = (settings.inputVolume ?? 0) + '%';
            if (ovv)
                ovv.textContent = (settings.outputVolume ?? 0) + '%';
            if (svv)
                svv.textContent = String(settings.sensitivityThreshold ?? 0);
