...
(function () {
    ...
    function sendLog(level, context, message, data) {
        if ((LEVEL_RANKS[level] ?? 0) < MIN_LEVEL_RANK)
            return;
        const payload = {
    ...
    function createLogger(context) {
        return {
            debug: (msg, data) => sendLog('debug', context, msg, data),
            info: (msg, data) => sendLog('info', context, msg, data),
            warn: (msg, data) => sendLog('warn', context, msg, data),
            error: (msg, data) => sendLog('error', context, msg, data),
        };
    }
