 ...
 */
'use strict';
const _voiceLog = window.emberLog
    ? window.emberLog.createLogger('Voice')
    : {
        debug: () => { },
        info: () => { },
        warn: (m, d) => console.warn('[Voice]', m, d ?? ''),
        error: (m, d) => console.error('[Voice]', m, d ?? ''),
    };
class VoiceManager {
        ...
        this.peerConnection.ontrack = (event) => {
            ...
            if (event.track.kind === 'audio') {
                if (!this.remoteStreams.has(stream.id)) {
                    ...
                    if (!this.isDeafened)
                        this._playRemoteStream(stream.id, stream);
                }
            }
    ...
    handleSpeakingEvent(userId, _level, isSpeaking) {
    ...
    handleMessage(msg) {
        ...
        switch (msg.type) {
            ...
            case 'voice_ice_candidate':
                ...
                break;
            case 'voice_speaking':
                this.handleSpeakingEvent(String(msg.payload['user_id'] ?? ''), Number(msg.payload['level'] ?? 0), Boolean(msg.payload['is_speaking']));
                break;
